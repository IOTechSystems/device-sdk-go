...
// version number, config profile, config directory and Driver, which cannot be nil.
// Note - this function is a singleton, if called more than once,
// it will always return an error.
func NewService(serviceVersion string, confProfile string, confDir string, proto ds_models.ProtocolDriver) (*Service, error) {
	if svc != nil {
