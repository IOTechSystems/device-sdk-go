	...
	for _, cv := range results {
		...
		if len(cv.RO.Mappings) > 0 {
			...
			if ok {
				...
				common.LoggingClient.Error(fmt.Sprintf("Handler - execReadCmd: Resource Operation (%v) mapping value (%s) failed with the mapping table: %v", cv.RO, cv.String(), cv.RO.Mappings))
				transformsOK = false
			}
		}
