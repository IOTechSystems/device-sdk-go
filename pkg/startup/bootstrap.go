...
func Bootstrap(serviceVersion string, driver ds_models.ProtocolDriver) {
	...
	if err := startService(serviceVersion, confProfile, confDir, driver); err != nil {
		fmt.Fprintf(os.Stderr, "error: %v\n", err)
		os.Exit(1)
	}
}

func startService(serviceVersion string, confProfile string, confDir string, driver ds_models.ProtocolDriver) error {
	s, err := device.NewService(serviceVersion, confProfile, confDir, driver)
	if err != nil {
		return err
	}
